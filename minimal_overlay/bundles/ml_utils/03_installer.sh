...
# This script installs Minimal Linux on Ext2 partition.
cat << CEOF > sbin/ml-install
#!/bin/sh
...
if [ "\$PRINT_HELP" = "true" ] ; then
  ...
  This is the Minimal Linux installer. Requires root permissions.

  Usage: ml-install DEVICE

  DEVICE    The device where Minimal Linux will be installed. Specify only
  ...
  The above example installs Minimal Linux  on '/dev/sdb'.
...
cat << DEOF
  ...
  Minimal Linux will be installed on device '/dev/\$1'. The device will be
...
# Possible solution 2 - drop 32-bit ML and provide 64-bit with multi-arch.
...
echo "Minimal Linux installer has been generated."
